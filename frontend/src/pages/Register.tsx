...
export const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await authService.register({ name, email, password, role: 'PATIENT' });
      loginStore(response);
      ...
      <div className="hidden md:flex flex-col justify-between p-12 bg-gradient-to-br from-teal-900 via-slate-900 to-slate-950 relative overflow-hidden border-r border-slate-800">
        ...
        <div className="space-y-6 z-10 max-w-md">
          ...
          <p className="text-slate-400 leading-relaxed">
            Create an account in seconds to schedule bookings, view AI-generated visit summaries, and coordinate prescriptions easily.
          </p>
        </div>

        <div className="flex items-center space-x-2 text-sm text-slate-500 z-10">
          <Shield className="h-4 w-4 text-teal-500" />
          <span>Your medical data is HIPAA-secured</span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-8">
        <div className="w-full max-w-md space-y-8 glass p-8 rounded-2xl border border-slate-800">
          <div className="text-center">
            <UserPlus className="mx-auto h-12 w-12 text-teal-400" />
            <h2 className="mt-6 text-3xl font-extrabold text-white">Create account</h2>
            <p className="mt-2 text-sm text-slate-400">Register as a CareFlow patient</p>
          </div>
          ...
          <div className="text-center mt-4">
            ...
            <Link to="/login" className="text-sm font-bold text-teal-400 hover:text-teal-300">
              Sign in instead
            </Link>
          </div>
        </div>
